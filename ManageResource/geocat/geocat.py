...
class GeocatAPI():
    ...
    def get_metadata_languages(self, metadata: bytes) -> list:
        ...
        main_lang = xml_root.find("./gmd:language/gmd:LanguageCode", 
                        namespaces=constants.NS).attrib["codeListValue"]

        languages.append(constants.LANG_ISO[main_lang])

        for lang in xml_root.findall("./gmd:locale//gmd:LanguageCode", namespaces=constants.NS):
            if constants.LANG_ISO[lang.attrib["codeListValue"]] not in languages:
                languages.append(constants.LANG_ISO[lang.attrib["codeListValue"]])
